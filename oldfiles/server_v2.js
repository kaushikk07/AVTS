...
app.get('/search/:licensePlate/:date?', (req, res) => {
    ...
    // Query to search data based on license plate number and date if provided
    let query = 'SELECT d.license_plate, d.date, d.time, c.camera_no, c.lat, c.lang ' +
                'FROM detections as d, cameras as c ' +
                'WHERE d.camera_no = c.camera_no ' +
                'AND license_plate = ? ';
  
    const queryParams = [licensePlate];
  
    if (date) {
      query += 'AND DATE(d.date) = DATE(?) ';
      queryParams.push(date);
    }
  
    query += 'ORDER BY d.time';
